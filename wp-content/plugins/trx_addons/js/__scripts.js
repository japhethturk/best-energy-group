(function() {
    ...
    window.trx_addons_is_images_loaded = function(cont) {
        ...
        cont.find('img').each(function() {
            ...
            if (typeof img.naturalWidth == 'number' && typeof img.naturalHeight == 'number') complete = !(img.naturalWidth == 0 && img.naturalHeight == 0);
    ...
    window.trx_addons_elementor_prepare_animate_items = function(force) {
        ...
        jQuery('[class*="animation_type_"]:not(.animation_type_block)' + (!force ? ':not(.animated-separate)' : '')).each(function() {
            ...
            if (sc_name) {
                ...
                sc_name = is_section && sc.find('>.elementor-container>.elementor-row>.elementor-column,>.elementor-row>.elementor-column,>.elementor-column').length > 1 ? '>.elementor-container>.elementor-row>.elementor-column,>.elementor-row>.elementor-column,>.elementor-column' : '[class*="_column-"]';
            }
        ...
        if (data['bg_text']) {
            ...
            if (marquee_speed > 0 && cid === '') {
                var marquee_wrap = row_cont.find('.trx_addons_marquee_wrap').eq(0),
                    marquee_chars = Math.min(100, data['bg_text_effect'] == 'none' ? trx_addons_clear_tags(data['bg_text']).length : marquee_wrap.find('.trx_addons_bg_text_char').length);
                setTimeout(function() {
                            ...
                            if (inner_obj.length === 0) {
                                ...
                                if (inner_obj.length > 0) {
                                    inner_obj.each(function(idx) {
                                        inner_obj.eq(idx).html(params.text == 'chars' ? self.wrap_chars(inner_obj.eq(idx).html(), true) : self.wrap_words(inner_obj.eq(idx).html()));
                                    });
