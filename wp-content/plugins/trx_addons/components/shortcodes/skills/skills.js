(function() {
    ...
    $document.on('action.init_trx_addons', function() {
        ...
        update_jquery_links();
        $document.on('action.init_hidden_elements', trx_addons_sc_skills_init).on('action.scroll_trx_addons', trx_addons_sc_skills_init).on('action.resize_trx_addons', trx_addons_sc_skills_resize);
        ...
        function trx_addons_sc_skills_init(e, container) {
            ...
            $skills_items.each(function(idx) {
                ...
                total.data('decimals', Math.max(0, start != parseInt(start, 10) ? ('' + start).length - 1 - ('' + parseInt(start, 10)).length : 0, stop != parseInt(stop, 10) ? ('' + stop).length - 1 - ('' + parseInt(stop, 10)).length : 0));
                if (type == 'bar') {
